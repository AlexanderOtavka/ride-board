...
echo
echo "Making yarn files world-writable..."
echo
chmod a+w yarn.lock
chmod a+w package.json
...
IFS=$' '
eval "docker-compose run web yarn $*"
IFS=$'\n\t'
...
CONTAINER_ID="$(docker create ride-board_web)"
...
docker cp $CONTAINER_ID:/srv/yarn.lock ./yarn.lock
